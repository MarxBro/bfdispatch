...
my $input_archivo = $ARGV[0] || die 'error: no se paso un archivo en la entrada';
...
sub mv_cbz_der {
    $PUNTERO_TAPE++;
    $TAPE[$PUNTERO_TAPE] = 0 unless (exists($TAPE[$PUNTERO_TAPE])); # crear nuevo elemento si es necesario
    return 1;
}
...
sub loop_start {
    ...
    if ($TAPE[$PUNTERO_TAPE] == 0){
        #// go one more the next ]    
        #$PUNTERO_TAPE = buscar_code(']',1);
